...
def main():
    ...
    parser.add_argument("file", help="The ASP encoding file to solve")
    parser.add_argument("-t", "--timeout", type=int, default=2000, 
                        help="Timeout in seconds for the solver (default: 60)")
    ...
    print(f"Solving for Optimum (Timeout: {args.timeout}s)...")

    # --- THREADING & TIMEOUT LOGIC ---
    def interrupt_solver():
        print(f"\n⏳ [Timeout] {args.timeout} seconds reached! Interrupting solver...")
        ctl.interrupt()

    # Start the background timer thread
    timer = threading.Timer(args.timeout, interrupt_solver)
    timer.start()

    try:
        with ctl.solve(yield_=True) as handle:
            for model in handle:
                # Clingo naturally yields progressively better models.
                # By overwriting here, we always hold the best model found so far.
                optimal_model_data = extract_data(model)
                if hasattr(model, 'cost') and model.cost:
                    # model.cost is a list containing the values of all #minimize levels
                    optimal_cost = list(model.cost) 

            # Once the loop finishes (either naturally or via interrupt), check the status
            result = handle.get()
            
    finally:
        # Crucial: Cancel the background timer if the solver finishes early!
        timer.cancel()
    ...
    if optimal_model_data:
        ...
        if result.interrupted:
            print(f" ⚠️ BEST FOUND BEFORE TIMEOUT. Costs (Levels 2, 1): {optimal_cost}")
        else:
            print(f" 🏆 OPTIMUM PROVEN! Costs (Levels 2, 1): {optimal_cost}")
        print("*"*50)

        run_audit(optimal_model_data)
        generate_graph(optimal_model_data, args.file)
    else:
        print("\n❌ No models were found within the time limit or the problem is UNSATISFIABLE.")
