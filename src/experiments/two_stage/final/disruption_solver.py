...
Sweeps div_weight across a range, and for each weight:
  ...
  Step 3 — Solve min_max.lp for that arc → get regret + deviation
  Step 4 — Append row to results, write CSV at end
...
def main() -> None:
    ...
    for w in args.weights:
        ...
        if robust is None:
            ...
            # Extract recovery cost from the shown atom (most reliable)
            rec_cost  = robust.get("recovery_cost")
            regret    = robust.get("regret")

            # If regret atom not shown, compute from recovery - nominal
            if regret is None and rec_cost is not None:
                regret = rec_cost - nominal_cost

            # Load deviation is the @0 component of the cost vector
            cost_vec  = robust.get("cost_vec", [])
            deviation = cost_vec[1] if len(cost_vec) > 1 else None   # @0 component
        ...
        results.append(row)
        write_csv(results, args.output)   # write after every weight so progress is saved
