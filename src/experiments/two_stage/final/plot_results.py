...
def plot_pareto_aggregate(df: pd.DataFrame, outdir: Path) -> None:
    print("Plotting aggregate Pareto front")
    ok = df[(df["status"] == "OK") &
            df["transport_cost"].notna() &
            df["dominant_count"].notna()].copy()
    if ok.empty:
    ...
    # ── Pareto front across all instances ───────────────────────────────
    ...
    fig.suptitle("Pareto Front: Transport Cost vs Dominant Count",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-dir", type=Path,
                   default=Path("src/experiments/two_stage/final/results/plots/paper"),
