#!/usr/bin/env bash
# Run scalability experiments size-by-size, with ntfy.sh pings.
#
...
#   NTFY_TOPIC=racquel-exp-xxxx ./run_all.sh small medium   # subset
...
SOLVERS="${SOLVERS:-naive_oneshot clingcon_oneshot naive_twostage clingcon_twostage}"
...
run_size() {
    local size="$1" time_limit_flag="$2" time_limit_val="$3"
    local tag="scalability_${size}_${RUN_TAG}"
    local log="$LOG_DIR/run_${tag}.log"
    local t0 t1 elapsed exit_code

    echo
    echo "============================================================"
    echo "  scalability size:  $size"
    echo "  run_tag:           $RUN_TAG"
    echo "  out files:         $RESULTS_DIR/{scalability_raw,scalability_summary}_${tag}.csv"
    echo "  log:               $log"
    echo "============================================================"

    ntfy "scalability:$size start" default "rocket" \
         "host=$(hostname) tag=$RUN_TAG reps=$REPS bins=$NUM_BINS"
    ...
    local raw="$RESULTS_DIR/scalability_raw_${tag}.csv"

    if [ $exit_code -eq 0 ]; then
        ntfy "scalability:$size done" default "white_check_mark" \
        ...
        ntfy "scalability:$size FAILED exit=$exit_code" high "warning,skull" \
...
run_one() {
    case "$1" in
        paper)        run_size paper        --time-paper        "$TIME_PAPER" ;;
        small)        run_size small        --time-small        "$TIME_SMALL" ;;
        medium)       run_size medium       --time-medium       "$TIME_MEDIUM" ;;
        large)        run_size large        --time-large        "$TIME_LARGE" ;;
        xlarge)       run_size xlarge       --time-xlarge       "$TIME_XLARGE" ;;
        industrylite) run_size industrylite --time-industrylite "$TIME_INDUSTRYLITE" ;;
...
ntfy "scalability:batch start" default "rocket" \
